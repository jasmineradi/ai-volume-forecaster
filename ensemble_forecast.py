...
class EnsembleForecaster:
    ...
    def __init__(self, data_path):
        ...
        self.stat_forecaster = VolumeForecaster(data_path)
        self.ml_forecaster = MLForecaster(self.data)
    ...
    def create_ensemble_forecast(self):
        ...
        stat_adjustment = (
            self.weights['moving_average'] * ma_forecast +
    ...
    def create_visualization_suite(self, forecast_df):
        ...
        axes[1, 0].set_xticklabels(
            [d.strftime('%b %y') for d in monthly_totals.index],
            rotation=45
        )
        axes[1, 0].grid(True, alpha=0.3, axis='y')

        # Plot 4: Uncertainty Analysis
        uncertainty = (forecast_df['upper_bound'] - forecast_df['lower_bound']) / \
            forecast_df['weighted_ensemble'] * 100
        axes[1, 1].plot(forecast_df['date'], uncertainty,
                        color=colors['forecast'], linewidth=2)
        axes[1, 1].fill_between(forecast_df['date'], uncertainty,
...
def main():
    ...
    # Initialize ensemble system
    ensemble = EnsembleForecaster('data/historical_volumes.csv')
